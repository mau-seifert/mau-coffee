...
1. **Git push to `main`** → Tangled.sh sends a POST webhook to `https://domain/webhook.php`.
...
curl -k -X POST https://domain/webhook.php \
  ...
  -H "X-Tangled-Signature-256: sha256=$SIGNATURE" \
  -d "$PAYLOAD"
```
...
- **403 Forbidden**  
  Signature mismatch. Check the secret and the header name. Tangled uses `X-Tangled-Signature-256`, not `X-Hub-Signature-256`.
